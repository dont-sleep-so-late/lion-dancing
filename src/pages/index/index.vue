...
<template>
  <view class="page">
    ...
    <view v-if="renderPageStatus[1]" class="page__container" :style="pageContainerStyle(1)">
      <scroll-view class="scroll-view" scroll-y @scroll="scrollViewScrollHandle">
        <PageB />
      </scroll-view>
    </view>
    <view v-if="renderPageStatus[2]" class="page__container" :style="pageContainerStyle(2)">
      <scroll-view class="scroll-view" scroll-y @scroll="scrollViewScrollHandle">
        <PageC />
      </scroll-view>
    </view>
    <view v-if="renderPageStatus[3]" class="page__container" :style="pageContainerStyle(3)">
      <scroll-view class="scroll-view" scroll-y @scroll="scrollViewScrollHandle">
        <PageD />
      </scroll-view>
    </view>
    <view v-if="renderPageStatus[4]" class="page__container" :style="pageContainerStyle(4)">
      <scroll-view class="scroll-view" scroll-y @scroll="scrollViewScrollHandle">
        <PageE />
      </scroll-view>
    </view>
  ...
  <!-- 底部导航栏 -->
  <TnTabbar v-model="currentTabbarIndex" fixed frosted switch-animation :placeholder="false" font-size="20"
    @change="tabbarChangeHandle">
    <TnTabbarItem v-for="(item, index) in tabbarData" :key="index" :text="item.text" :icon="item.icon"
      :active-icon="item.activeIcon" :bulge="index === 2" :icon-size="index === 2 ? '56' : ''" />
  </TnTabbar>
</template>
